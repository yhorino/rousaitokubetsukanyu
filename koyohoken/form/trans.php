...
//include('session_check.php');
  ...
  require_once("../../form/bin/sf_Api_sb.php");
  init();
  sf_login();
...
switch($_POST['pagename']){
 ...
 case 'mitsumori.php':
 {
  ...
  if(isset($_SESSION['jimuKaiinNo__c']) && $_SESSION['jimuKaiinNo__c'] != '') {
   $result_kaisya = null;
   $result_kaisya = (array)getKaisyabyKaiinNo($_SESSION['jimuKaiinNo__c']);
   $row = (array)$result_kaisya["fields"];
   
   $jimuRoudouhokenBangou5__c = $row['jimuRoudouhokenBangou5__c'];
  ...
  if($ret_getrjcdata['kaisyamei'] != null && $ret_getrjcdata['kaisyamei'] != ""){
   ...
   $_SESSION['jimuChinginShimeShiharaibi__c'] = $ret_getrjcdata['jimuChinginShimeShiharaibi__c'];
  }
...
*/
 ...
 'NyusyaDate__c'=>urlencode($items['nyusyadate_y'.$i].'/'.$items['nyusyadate_m'.$i].'/'.$items['nyusyadate_d'.$i]),
 'KyuyoKubun__c'=>urlencode($items['kyuyokubun'.$i]),
 'Mobile__c'=>urlencode($items['mobile'.$i]),
 //'debug' => '1',
 //'debugEmail' => urlencode("xxx@xxxx"),
 );
 foreach($fields as $key=>$value) { $fields_string .= $key.'='.$value.'&'; }
